...
BEGIN
    ...
    SET @TEAM_CONC = (SELECT CONCURRENCY_CONTROL_NUMBER
    FROM TRA_USER_ACTIVITY
    WHERE TRA_USER_ACTIVITY.USER_ACTIVITY_ID = (SELECT USER_ACTIVITY_ID
    FROM inserted));
    DECLARE @INSERTED_CONC INT;
    SET @INSERTED_CONC = (SELECT CONCURRENCY_CONTROL_NUMBER
    FROM inserted);
...
BEGIN
    ...
    FROM dbo.TRA_USER_ACTIVITY
    WHERE @@ROWCOUNT > 0 AND USER_ACTIVITY_ID = SCOPE_IDENTITY()
